...
Each constraint here gives a \(\pqty{n - 1}\)-dimensional subspace as the boundary (and hence the feasible set hence must be convex). Instead, we optimise in an unconstrained setup
\[
...
\]
for \(x < 0\). This is the \emph{logarithmic barrier function}.
